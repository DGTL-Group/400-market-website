...
svg = svg.replace(
  ...
  (_m, inner) => {
    ...
    const label = `
        <text class="fp-food-court-label" x="584.066" y="472" font-size="16">FOOD COURT</text>`
...
const styleBlock = `  <style>
    ...
    rect {
      ...
      stroke-width: 1;
      vector-effect: non-scaling-stroke;
      /* Transitions live on the base rule (not on
         [data-filter="dim/match"]) so they fire BOTH into AND out
         of the filtered state. When React removes data-filter on
         clear, the element falls back to base styles — and base
         is where the transition has to live or the snap-back is
         instant. Deliberately NOT transitioning stroke color:
         when the orange match stroke fades back to black at the
         same time as opacity, you get a brown mid-transition
         that looks broken. Color flips instantly, opacity and
         stroke-width fade smoothly. */
    ...
    /* On become-a-vendor the palette FLIPS: the thing the visitor is
       hunting for (open booths) becomes the eye-catching yellow, and
       already-rented booths fade back to white so they read as
       "context, not target". Reserved + blocked stay as-is.

       !important is here because Chrome's transition machinery on the
       blanket "rect { fill: #FFFFFF; transition: fill 150ms; }" rule
       was pinning the fill at the start value even though our class-
       scoped selectors are higher specificity. Forcing the cascade
       with !important is the path of least resistance — the rest of
       the SVG palette is static so there is no cross-rule fight to
       worry about. */
    .fp-mode-become-a-vendor rect[data-booth][data-status="available"] {
      fill: #F7D117 !important;
      /* Stroke deliberately NOT set here. The default rect rule
         already supplies #2C2C2C, and pinning it again with
         !important + a 0,3,2-specificity selector would beat the
         filter-match orange outline — which made "Available
         booth" highlights render with a black stroke instead of
         the expected mango. */
    }
    .fp-mode-become-a-vendor rect[data-booth][data-status="rented"] {
      fill: #FFFFFF !important;
    }
    .fp-mode-become-a-vendor path[data-fp-merged],
    .fp-mode-become-a-vendor rect[data-fp-merged] {
      fill: #FFFFFF !important;
    }
    .fp-mode-become-a-vendor .fp-food-court-booth[data-status="available"] {
      fill: #F7D117 !important;
    }
    .fp-mode-become-a-vendor .fp-food-court-booth[data-status="rented"] {
      fill: #FFFFFF !important;
    }

    /* Dim everything that is NOT an available booth so the yellow
       targets pop. Rented (white), reserved (orange), blocked (gray),
       merged clusters, and food-court rented all fade back. The dev
       labels and the merged vendor name fade with them so the noise
       follows the booth. Filter overlay (the search/legend pulse)
       still wins because it sets data-filter and has its own opacity
       rules with higher specificity in this same stylesheet. */
    .fp-mode-become-a-vendor rect[data-booth][data-status="rented"],
    .fp-mode-become-a-vendor rect[data-booth][data-status="reserved"],
    .fp-mode-become-a-vendor rect[data-booth][data-status="blocked"],
    .fp-mode-become-a-vendor path[data-fp-merged],
    .fp-mode-become-a-vendor rect[data-fp-merged],
    .fp-mode-become-a-vendor .fp-food-court-booth[data-status="rented"],
    .fp-mode-become-a-vendor .fp-food-court-booth[data-status="reserved"] {
      opacity: 0.55;
    }
    .fp-mode-become-a-vendor text[data-fp-merged],
    .fp-mode-become-a-vendor text[data-fp-label] {
      opacity: 0.55;
    }
    ...
    /* Merged adjacent-booth shape — a polygon path tracing the
       cluster's outer perimeter (handles L/T/U-shapes). Same
       stroke width as a single booth so the whole map reads with
       a consistent line weight regardless of which booths are
       grouped. */
    path[data-fp-merged],
    rect[data-fp-merged] {
      fill: #F7D117;
    ...
    /* Search + legend filter states. React stamps data-filter="match"
       ...
       results. */
    /* Filter opacity wins via !important because the mode-level
       become-a-vendor rule (rented at 0.55) has higher selector
       specificity. Without these the orange match outline rendered
       at 0.55 on rented booths in become-a-vendor — the user
       clicking "Active vendors" got a sad, washed-out highlight
       instead of the clean pop they expected.
       Transitions live on the base rect/path/text rules so they
       fire on filter clear too. */
    [data-filter="dim"] {
    ...
    /* !important here because the rented-booth defaults have higher
       specificity (rect[data-booth][data-status="rented"]) than the
       match selector — without it, single rented rects keep their
       dark stroke while merged paths get the orange one, producing
       a mix of black and orange outlines when "Rented" is clicked.
       Static stroke-width (no pulse) so every matched shape reads
       at the SAME width — pulsing produced visibly different stroke
       widths per element because each element animation started
       at the moment data-filter=match was stamped, and rects vs
       cluster paths get the attribute in slightly different
       React-effect ticks, leaving the pulses out of phase. */
    rect[data-filter="match"],
    path[data-filter="match"] {
      stroke: #E57200 !important;
    ...
    /* Food-court booths — same status palette as regular booths plus a
       mango stroke so the whole zone reads as one block. Same
       stroke-width as every other booth for visual consistency. */
    .fp-food-court-booth {
      fill: #FFFFFF;
      stroke: #E57200;
      stroke-width: 1;
    ...
    /* FOOD COURT label — dropped in at step 4 below, positioned over
       the Food-Court1 zone rect. Dark fill so it's legible against
       the cream floor; weight 900 for a chunkier, easier-to-read
       headline. */
    .fp-food-court-label {
      fill: #2C2C2C;
      font-family: var(--font-metafora), 'METAFORA', 'DM Sans', sans-serif;
      font-weight: 900;
