...
|---|---|---|---|---|---|
| 1 | **PostgreSQL** (self-hosted, VPS) | Payload CMS database — replaces Neon | Installed on VPS ✓ — `DATABASE_URI` set in `.env.local` | DGTL | [x] |
| 2 | **Cloudinary** | Image CDN and asset storage | Cloud Name + API Key + Secret confirmed ✓ — set in `.env.local` | DGTL | [x] |
